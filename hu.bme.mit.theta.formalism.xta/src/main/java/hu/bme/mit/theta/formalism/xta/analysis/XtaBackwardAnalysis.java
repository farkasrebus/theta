...
public class XtaBackwardAnalysis<S extends State, P extends Prec> implements Analysis<XtaState<S>, XtaAction, P> {
	...
	private XtaBackwardAnalysis(final XtaSystem system, Set<List<Loc>> initConfigs, final Analysis<S, ? super XtaAction, ? super P> analysis) {
		checkNotNull(system);
		checkNotNull(analysis);
		partialOrd = XtaOrd.create(analysis.getPartialOrd());
		initFunc = XtaBackwardsInitFunc.create(system, initConfigs, analysis.getInitFunc());
	...
	public static <S extends State, P extends Prec> XtaBackwardAnalysis<S, P> create(final XtaSystem system,
		...
		return new XtaBackwardAnalysis<>(system, initConfigs, analysis);
	}
